...
def normalize_cells_to_reference(
    ...
    if scale is None:
        scale = normalization_scale(cells, target, cell_margin)
    return [normalize_cell_to_geometry(cell, target, scale) for cell in cells]


def normalization_scale(
    cells: list[Image.Image], target: CellGeometry, cell_margin: int
) -> float:
    ...
    scale_limits = [
        ...
        (CELL_WIDTH - 2 * cell_margin) / max_width,
        (CELL_HEIGHT - 2 * cell_margin) / max_height,
        1.0,
    ]
    margin = cell_margin
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Minimum alpha-bbox margin used while registering look cells (default: 6).",
    )
    parser.add_argument(
    ...
    if args.registered_row_9:
        ...
        if target is None:
            raise SystemExit("neutral reference cell must contain visible pixels")
        scale = normalization_scale(cells, target, args.cell_margin)
        cells = normalize_cells_to_reference(cells, neutral, scale, args.cell_margin)
        validate_normalized_look_cells(
            cells,
            0,
            args.edge_margin,
