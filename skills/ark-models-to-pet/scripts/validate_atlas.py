...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Require the nonzero-alpha bbox of every used cell to stay at least this "
            "many pixels from every edge (default: 0)."
        ),
    )
        ...
        for column_index in range(COLUMNS):
            ...
            alpha_margins = (
                [
                    alpha_bbox[0],
                    alpha_bbox[1],
                    CELL_WIDTH - alpha_bbox[2],
                    CELL_HEIGHT - alpha_bbox[3],
                ]
                if alpha_bbox
                else None
            )
            cell_info["alpha_bbox"] = list(alpha_bbox) if alpha_bbox else None
            cell_info["alpha_margins"] = alpha_margins
            ...
            if (
                used
                and alpha_margins is not None
                and min(alpha_margins) < args.min_alpha_margin
            ):
                errors.append(
                    f"{state} row {row_index} column {column_index} violates the "
                    f"{args.min_alpha_margin}px alpha margin: {alpha_margins}"
                )
